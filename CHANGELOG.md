...
## v0.7.2 — Update mechanics + notifications

### Added
- Self-update system: `/api/update/check`, `/api/update/install`, stub supervisor
- Update notification: "U" badge on bell icon, banner in notifications panel
- Click notification → opens Settings/About directly
- `updater.sh` — standalone download + extract + restore
- Supervisor auto-checks GitHub every 30 min
- Backup/rollback to `old-releases/` (keeps 3 versions)
- Readiness timeout 120 sec for low-RAM hardware

### Changed
- `meshcorium-launcher.sh`: `--supervise` mode, `fetch_all_releases`, `find_next_release`, `auto_check_updates`, `perform_supervised_update`
- `meshcorium_web.py`: update API endpoints, `_UpdateInstallHandler`, `_build_update_check_payload`, `_read_flag_*`, flag-file IPC
- `web/src/views/SettingsView.vue`: update panel in About section, `loadUpdateCheck()`, `installUpdate()`, `pollUpdateStatus()`
- `web/src/components/layout/ConnectedShellLayout.vue`: update check polling, "U" badge
- `web/src/components/messages/MessagesNotificationsSheet.vue`: update banner at top
- `web/src/views/MessagesView.vue`: update check + notification
- `web/src/stores/session.js`, `web/src/session.js`: `api()` auto-stringifies JSON body
- `web/src/styles.css`: update badge, update pill CSS
- `web/src/i18n/messages/ru.js`, `en.js`: update notification strings

### Fixed
- `session.api()`: `body: {}` not stringified → JSONDecodeError
- `fetch_all_releases`: BusyBox `sort -V` returns wrong order
- `auto_check_updates` / `find_next_release`: wrong next-version logic
- Backup `cp -a` recursion into self
- `update_available` flag file permissions (root-owned)
- API cache race condition after update
- Stale JS bundles in `dist/` (`--emptyOutDir false`)

### Preserved
- `data/`, `logs/`, `other/` survive updates (user data, wallpapers)
- `updater.sh` merges missing `client_settings.json` keys
