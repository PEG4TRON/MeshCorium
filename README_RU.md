...
Статус релиза `MeshCorium v0.6.0 -- Docker + USB + BLE`:
...
## Главное отличие `v0.6.0` от `v0.5.3`
...
В `v0.6.0` функционально добавлены и расширены:
...
В `v0.6.0` Docker-вариант приведён к текущему коду приложения:
...
3. Распаковать `MeshCorium v0.6.0 -- Docker + USB + BLE` в новый каталог рядом со старой установкой.
...
- Старый каталог лучше оставить как резервную копию до подтверждения, что `v0.6.0` работает штатно.
