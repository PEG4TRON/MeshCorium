...
Статус релиза `MeshCorium v0.7.0 -- Docker + USB + BLE + WIFI/LAN`:
...
- `Docker Compose` — вариант эксплуатации остаётся рядом с обычным launcher/systemd сценарием
...
В `v0.7.0` Docker-вариант приведён к текущему коду приложения:
...
3. Распаковать `MeshCorium v0.7.0 -- Docker + USB + BLE + WIFI/LAN` в новый каталог рядом со старой установкой.
...
- Старый каталог лучше оставить как резервную копию до подтверждения, что `v0.7.0` работает штатно.
