...
Примечание по текущей dev-версии:

- экспериментальная интеграция `Wi-Fi / TCP` сейчас в работе;
- в активном development code уже есть backend TCP transport plumbing и ручной `host:port` connect-flow;
- в ветку `dev` добавлен тестовый функционал `Wi-Fi / LAN` для проверки и дальнейшей интеграции;
- эта часть пока не прошла release-валидацию и не должна считаться частью опубликованного профиля `v0.6.1`.

Статус релиза `MeshCorium v0.6.1 -- Docker + USB + BLE`:
...
- `Wi-Fi / LAN` — в `dev` доступен тестовый `TCP host:port` transport, но он всё ещё экспериментальный и не прошёл release-валидацию
...
## Главное отличие `v0.6.1` от `v0.6.0`

В `v0.6.1` дополнительно добавлены:

- браузерные уведомления по общему приросту unread всех типов после текущих mute- и owner-scope фильтров;
- общий unread-бейдж в заголовке вкладки браузера;
- обновлённые Docker release-label и image metadata для `v0.6.1`.

## Ключевые отличия `v0.6.x` от `v0.5.3`
...
В `v0.6.1` Docker-вариант приведён к текущему коду приложения:
...
3. Распаковать `MeshCorium v0.6.1 -- Docker + USB + BLE` в новый каталог рядом со старой установкой.
...
- Старый каталог лучше оставить как резервную копию до подтверждения, что `v0.6.1` работает штатно.
...
- Текущий релизный профиль поддерживает `USB serial`, `BLE` и Docker Compose вариант эксплуатации.
