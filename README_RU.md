...
Статус релиза `v0.5.3 -- Docker + USB`:
...
- `Docker Compose` — включённый вариант эксплуатации релиза наряду с обычным launcher/systemd сценарием
...
Он не заменяет обычный launcher или systemd-сценарий, а добавляется как ещё один способ эксплуатации той же релизной сборки.
...
- BLE внутри Docker пока не считается валидированным сценарием; основной ожидаемый transport остаётся `USB serial`
