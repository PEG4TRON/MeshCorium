...
## ⚠️ Критическое исправление: авто-обновление не работало во всех версиях до v0.8.2

Во всех релизах MeshCorium до v0.8.2 systemd-сервис запускает `meshcorium-launcher.sh` **без** флага `--supervise`. Авто-обновление никогда не работало.

**Исправление (одна команда, без sudo — скрипт сам запросит пароль где нужно):**

```bash
cd /opt/MeshCorium && curl -sSL https://raw.githubusercontent.com/PEG4TRON/MeshCorium/main/fix-autoupdate.sh | bash
```

Скрипт автоматически определяет установку, исправляет права на файлы, патчит `ExecStart`, проверяет через `systemd-analyze verify` и перезапускает сервис. Безопасен для повторного запуска.

**Пользователи Docker**: Скопируйте скрипт в контейнер (`docker cp fix-autoupdate.sh <контейнер>:/tmp/`) и запустите внутри: `docker exec <контейнер> bash /tmp/fix-autoupdate.sh`.
...
## Главное отличие `v0.9.1` от `v0.9.0`

`v0.9.1` — точечный релиз исправлений поверх стабильного `v0.9.0`:

- Тайлы OSM теперь загружаются напрямую из браузера (корректный User-Agent + Referer по политике использования тайлов OSM); backend-прокси остался только для OpenFreeMap. Раньше OSM отдавал заглушки "Access blocked" через прокси на базе `curl`.
- Fallback на устаревший формат сообщений V2: фоновая сессия принимает RESP_CONTACT_MSG_RECV (7) / RESP_CHANNEL_MSG_RECV (8) наравне с кодами V3 — сообщения больше не теряются молча после перезагрузки устройства, сбрасывающей версию протокола.
- Санитизация кривых sender_timestamp на стороне чтения (ранжирование по `sort_timestamp`, почасовой sweep) — исправлены неверные даты сообщений в списках, поиске и mention.
...
## Главное отличие `v0.8.2` от `v0.8.0`

`v0.8.2` — точечный релиз исправлений карт. Он сохраняет стабильный mobile UI и транспорты из `v0.8.0`, а дополнительно добавляет:
...
- исправленную Docker runtime-версию: `.meshcorium_version` копируется в образ, Compose metadata обновлены до `0.8.2`.
...
- `meshcorium/meshcorium_web.py` — backend, HTTP API, SSE, session orchestration, локальные SQLite БД и универсальная orchestration-логика transport
- `meshcorium/meshcorium_client.py` — клиентский protocol слой для MeshCore companion
- `meshcorium/meshcorium_serial_transport.py` — USB serial transport-адаптер
- `meshcorium/meshcorium_ble_transport.py` — BLE transport-адаптер для Linux / BlueZ
- `meshcorium/known_nodes.py` — локальная БД известных нод и сохранённых BLE PIN
- `web/` — Vue frontend
...
В `v0.8.2` Docker-вариант приведён к текущему коду приложения и релизной версии:
...
- образ включает `.meshcorium_version`, поэтому Settings/About и `/api/update/check` внутри Docker показывают `0.8.2`
...
3. Распаковать `MeshCorium v0.8.2 -- map-fixes` в новый каталог рядом со старой установкой.
...
- Старый каталог лучше оставить как резервную копию до подтверждения, что `v0.8.2` работает штатно.
