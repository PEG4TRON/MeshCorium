...
## Вариант через Docker Compose для следующего релиза

В ветке разработки теперь также есть Docker-вариант:

- `Dockerfile`
- `docker-compose.yml`

Он не заменяет обычный launcher или systemd-сценарий, а готовится как ещё один способ эксплуатации следующей релизной сборки.
