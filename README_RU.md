...
3. Распаковать `v0.5.1 -- USB` в новый каталог рядом со старой установкой.
...
- Если в `v0.5.0` у тебя встречалась ошибка
  ...
  то в `v0.5.1` добавлена backend-защита от этого startup-сценария.
- Старый каталог `v0.5.0` лучше оставить как резервную копию до подтверждения, что новая версия работает штатно.
