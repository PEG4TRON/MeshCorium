...
def _get_recent_repeater_count(session: BackgroundCompanionSession, now_epoch: int | None = None) -> int:
    with session.snapshot_lock:
        ...
        return len(session.repeater_tracker.full_keys)
...
def _build_session_snapshot(
    ...
    *,
    ...
    with session.snapshot_lock:
        ...
        _prune_repeater_tracker_locked(session, now_epoch=now_epoch)
        recent_repeaters_count = len(session.repeater_tracker.full_keys)
        snapshot = {
...
def _channel_runtime_idx(channel: object) -> int:
    if isinstance(channel, dict):
        return _channel_idx_value(channel)
    return _channel_idx_value(channel)
                    ...
                    while True:
                        ...
                        try:
                            ...
                            if kind == "set_node_location":
                                ...
                                owner_id = _normalize_owner_id(self_dict.get("public_key"))
                                with session.snapshot_lock:
                                    _safe_device_update(session, device_dict, new_self_info=dict(snapshot.get("self") or {}), context="set_node_location")
                                    session.self_info = self_dict
                            ...
                            if kind == "meshcore_params_snapshot":
                                ...
                                with session.snapshot_lock:
                                    _safe_device_update(session, dict(snapshot.get("device") or {}), new_self_info=dict(snapshot.get("self") or {}), context="meshcore_params_snapshot")
                            ...
                            if kind == "apply_meshcore_params":
                                ...
                                owner_id = _normalize_owner_id(self_dict.get("public_key"))
                                with session.snapshot_lock:
                                    _safe_device_update(session, device_dict, new_self_info=dict(snapshot.get("self") or {}), context="apply_meshcore_params")
                                    session.self_info = self_dict
                                    session.radio_stats = snapshot.get("radio_stats")
                                    session.self_telemetry = snapshot.get("self_telemetry")
                                    session.battery_info = snapshot.get("battery_info")
                            ...
                            response_queue.put({"ok": False, "error": str(exc), "error_code": "channel_conflict"})
...
def _run_command_via_background_session(port: str, command: dict, timeout_secs: float = 10.0) -> dict:
    ...
    if not response.get("ok"):
        error_message = str(
            response.get("error")
            or "background command failed"
        )
        error_code = str(
            response.get("error_code") or ""
        )

        if error_code == "channel_conflict":
            raise ChannelConflictError(error_message)

        raise MeshCoreError(error_message)
    return response


GITHUB_API_RELEASES = "https://api.github.com/repos/PEG4TRON/MeshCorium/releases"
MESHCORIUM_VERSION_FILE = ".meshcorium_version"
MESHCORIUM_UPDATE_AVAILABLE_FILE = ".meshcorium_update_available"
...
def _build_update_check_payload() -> dict:
    ...
    now = time.monotonic()
    if _UPDATE_CHECK_CACHE and (now - _UPDATE_CHECK_CACHE_TS) < _UPDATE_CHECK_CACHE_TTL:
        return dict(_UPDATE_CHECK_CACHE)
    ...
    payload = {
        ...
        "next_version": available.get("next", "") if available else "",
        "update_available": bool(available.get("next", "")),
...
class _UpdateInstallHandler:
    ...
    def handle(self):
        ...
        if update_state != "idle":
            self._handler._send_json(
                {"ok": False, "error": f"update already in progress ({update_state})"},
                status=HTTPStatus.CONFLICT,
            )
            return
        _write_pending_update(next_version)
        self._handler._send_json({
            "ok": True,
            "message": f"update to {next_version} started",
            "version": next_version,
        })


def _handle_update_install(self):
    _UpdateInstallHandler(self).handle()
...
def _resolve_after_history_offset(
    conn: sqlite3.Connection,
    table_name: str,
    where_sql: str,
    params: tuple,
...
def _safe_device_update(
    ...
    *,
    ...
    Compares nodes by public_key (self_info) — not model/firmware which can match
    across physically different nodes.

    Conditions for preserving old max_contacts:
    - same active session with existing device
    - old max_contacts > 0, new max_contacts == 0
    - same node (public_key match via _normalize_owner_id)
    ...
    old_self = dict(session.self_info or {})
    incoming_self = dict(new_self_info or {})

    old_public_key = _normalize_owner_id(old_self.get("public_key"))
    new_public_key = _normalize_owner_id(incoming_self.get("public_key"))

    same_node = bool(
        old_public_key
        and new_public_key
        and old_public_key == new_public_key
    )

    old_max = int(
        old_device.get("max_contacts")
        or old_device.get("max_contacts_base")
        or 0
    )
    new_max = int(
        new_device.get("max_contacts")
        or new_device.get("max_contacts_base")
        or 0
    )

    if same_node and old_max > 0 and new_max == 0:
        ...
        _logger.warning(
            ...
            "max_contacts dropped %s->%s; preserving previous value",
            context,
            old_max,
            new_max,
        )
    ...
    session.device = dict(new_device)
...
def _delete_channel_and_reload_with_client(
    ...
    existing_channel = next(
        (
            ...
            if _channel_runtime_idx(item) == int(channel_idx)
        ),
        None,
    )
    if existing_channel is None:
...
class MeshcoriumWebHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/update/check":
            self._send_json(_build_update_check_payload())
            return
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if parsed.path == "/api/node/name":
                ...
                if session:
                    with session.snapshot_lock:
                        _safe_device_update(session, device_dict, new_self_info=dict(snapshot.get("self") or {}), context="ws_node_name")
                        session.self_info = self_dict
            ...
            if parsed.path == "/api/node/self-location":
                ...
                if session:
                    with session.snapshot_lock:
                        _safe_device_update(session, device_dict, new_self_info=dict(snapshot.get("self") or {}), context="ws_other")
                        session.self_info = self_dict
                        session.radio_stats = snapshot["radio_stats"]
                        session.self_telemetry = snapshot["self_telemetry"]
                        session.battery_info = snapshot["battery_info"]
    ...
    def _handle_update_install(self) -> None:
        handler = _UpdateInstallHandler(self)
        handler.handle()
