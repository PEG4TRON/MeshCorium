...
const refreshButtonLabel = computed(() => {
  if (session.selectedTransportType === 'ble') {
    return session.loadingBleConnections ? t('connect.ble.scanning') : t('connect.ble.scan')
  }
      ...
      <div class="mc-connect-float-stack">
        <div class="mc-connect-float-card">
          <div class="mc-connect-float-primary">
            ...
            <button
              v-tooltip="session.selectedTransportType === 'ble' ? null : { content: session.selectedTransportType === 'wifi' ? t('connect.wifi.unavailableTitle') : t('common.refreshPorts'), theme: 'meshcorium-tooltip' }"
              class="mc-icon-button"
              :class="{ 'mc-icon-button--with-label': session.selectedTransportType === 'ble' }"
              type="button"
            ...
            >
              <SyncIcon
                class="mc-icon-button-glyph"
                :spinning="session.selectedTransportType === 'ble' ? session.loadingBleConnections : session.loadingPorts"
              />
