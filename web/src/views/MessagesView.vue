...
const bottomHistoryPaginationArmed = ref(true)
const suppressBottomPaginationUntil = ref(0)
let messageBottomPaginationIntentHost = null
...
const canLoadNewerMessages = computed(() => {
  return Boolean(
    workspaceMode.value === 'chat'
    && currentConversation.value
    && !loadingMessages.value
...
async function loadNewerMessages() {
  if (!canLoadNewerMessages.value) {
    return
  }
  bottomHistoryPaginationArmed.value = false
  suppressBottomPaginationUntil.value = Date.now() + 1200
  const newestMessageId = Number(messages.value[messages.value.length - 1]?.id || 0)
  ...
  try {
    if (selectedConversationKind.value === 'channel' && (selectedChannelIdx.value != null || selectedChannelIdentity.value)) {
      const params = new URLSearchParams({
        channel_idx: String(selectedChannelIdx.value ?? 0),
        limit: String(HISTORY_PAGE_SIZE),
        after_message_id: String(newestMessageId),
      })
      if (selectedChannelIdentity.value) {
        params.set('channel_identity', String(selectedChannelIdentity.value))
      }
      ...
      const newerMessages = sanitizeMessageList(data?.messages)
...
function handleMessageScroll() {
  ...
  if (
    host
    && isMessageScrollerNearBottom(40)
    && canLoadNewerMessages.value
    && bottomHistoryPaginationArmed.value
    && Date.now() >= suppressBottomPaginationUntil.value
  ) {
...
function markBottomPaginationIntentReady() {
  bottomHistoryPaginationArmed.value = true
}

function detachMessageBottomPaginationIntentListeners() {
  if (!(messageBottomPaginationIntentHost instanceof HTMLElement)) {
    messageBottomPaginationIntentHost = null
    return
  }
  messageBottomPaginationIntentHost.removeEventListener('wheel', markBottomPaginationIntentReady)
  messageBottomPaginationIntentHost.removeEventListener('touchstart', markBottomPaginationIntentReady)
  messageBottomPaginationIntentHost.removeEventListener('pointerdown', markBottomPaginationIntentReady)
  messageBottomPaginationIntentHost.removeEventListener('keydown', markBottomPaginationIntentReady)
  messageBottomPaginationIntentHost = null
}

function attachMessageBottomPaginationIntentListeners(host) {
  detachMessageBottomPaginationIntentListeners()
  if (!(host instanceof HTMLElement)) {
    return
  }
  host.addEventListener('wheel', markBottomPaginationIntentReady, { passive: true })
  host.addEventListener('touchstart', markBottomPaginationIntentReady, { passive: true })
  host.addEventListener('pointerdown', markBottomPaginationIntentReady, { passive: true })
  host.addEventListener('keydown', markBottomPaginationIntentReady)
  messageBottomPaginationIntentHost = host
}
...
watch(messageScroller, async () => {
  ...
  attachMessageBottomPaginationIntentListeners(messageScroller.value)
  resetVirtualMessageLayout()
  await nextTick()
  updateMessageScrollerMetrics()
  ensureMessageResizeObserver()
